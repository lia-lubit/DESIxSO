...
def build_covariance_from_data(
    ...
    if desired_spectra is None:
        ...
        sliced_f_map = ForecastMap(n_lens=lens_data.shape[1]-1, n_src=source_data.shape[1]-1, n_ell=n_ell, desired_spectra=desired_spectra)
        lens_tracers, source_tracers, cmb_tracer = build_tracers_from_data(cosmo, lens_data, source_data, magnification_bias_lenses)
        tracer_dict = build_tracer_dict(lens_tracers, source_tracers, cmb_tracer)
        noise_dict = build_noise_dict(sliced_f_map, ells, shot_noise_lens, shape_noise_source, cmb_noise_phi)
        sliced_spectra_dict = build_spectra_dict(cosmo, full_f_map, tracer_dict, ells, noise_dict, linear_emulator=linear_emulator, boost_emulator=boost_emulator)
